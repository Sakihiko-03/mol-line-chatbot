...
const Home = () => {
  ...
  const initLIFF = async () => {
    try {
      ...
      if (liff.isLoggedIn()) {
        ...
        sendMessage(profile.userId, 'ยืนยันตัวตนสำเร็จ');
      } else {
        liff.login();
      }
    } catch (error) {
  ...
  return (
    <div className="bg-gray-100 fixed w-[var(--inner-width)] h-[var(--inner-height)]">
      {liffProfile ? (
        <Login />
      ) : (
        <div
        ...
        >
          <svg
            aria-hidden="true"
            className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-[#9230F9]"
            viewBox="0 0 100 101"
            fill="none"
